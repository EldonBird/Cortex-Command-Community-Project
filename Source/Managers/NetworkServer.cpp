...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void NetworkServer::SendFrameSetupMsg(short player, bool useDelta, bool useInterlacing) {
		...
		msgFrameSetup.Interlaced = useInterlacing;
		msgFrameSetup.DeltaCompressed = useDelta;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int NetworkServer::SendFrame(short player) {
		...
		m_KeyframeCounters[player]++;
		bool isKeyframe = (m_KeyframeCounters[player] >= c_KeyframeInterval);
		if (isKeyframe) { m_KeyframeCounters[player] = 0; }
		// On a keyframe: send full non-delta, non-interlaced frame so the client can resync after
		// any dropped delta packets. The client clears its intermediate buffer on non-delta frames,
		// giving both sides identical state before delta resumes.
		bool useDelta = m_UseDeltaCompression && !isKeyframe;
		bool useInterlacing = m_UseInterlacing && !isKeyframe;
					...
					for (int bx = 0; bx <= boxedWidth; bx++) {
						...
						if (useDelta) {
							...
							if (m_UseDeltaCompression && prevLineBuffers) {
								unsigned char *prevLineBufferStart = prevLineBuffers + by * boxedWidth * boxMaxSize + bx * boxMaxSize;
								if (m_UseInterlacing) {
									// Keyframe sends all lines (non-interlaced); split them into the
									// even-half and odd-half slots that subsequent delta frames expect.
									int halfSize = maxWidth * (lineCount / 2);
									unsigned char *prevEven = prevLineBufferStart;
									unsigned char *prevOdd = prevLineBufferStart + halfSize;
									const unsigned char *src = m_PixelLineBuffer[player];
									for (int y = 0; y < lineCount; y++) {
										if (y % 2 == 0) { memcpy(prevEven, src, maxWidth); prevEven += maxWidth; }
										else             { memcpy(prevOdd,  src, maxWidth); prevOdd  += maxWidth; }
										src += maxWidth;
									}
								} else {
